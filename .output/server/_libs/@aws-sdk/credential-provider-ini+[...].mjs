...
import { Jt as init_dist_es, Y as parseKnownFiles, Yt as chain, Z as readFile, Zt as CredentialsProviderError, bn as init_dist_es$2, et as getProfileName, q as init_dist_es$1, qt as require_client, xn as HttpRequest } from "./client-s3+[...].mjs";
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
	const sourceProvidersMap = {
		EcsContainer: async (options) => {
			...
			const { fromContainerMetadata } = await import("../_17.mjs");
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
			return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
		},
		Ec2InstanceMetadata: async (options) => {
			logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
			const { fromInstanceMetadata } = await import("../_17.mjs");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData) => {
	...
	if (!options.roleAssumer) {
		const { getDefaultRoleAssumer } = await import("../_15.mjs").then((m) => /* @__PURE__ */ __toESM(m.default));
...
var LoginCredentialsFetcher = class LoginCredentialsFetcher {
	...
	async refresh(token) {
		const { SigninClient, CreateOAuth2TokenCommand } = await import("../_13.mjs").then((m) => /* @__PURE__ */ __toESM(m.default));
...
const resolveProcessCredentials = async (options, profile) => import("../_9.mjs").then(({ fromProcess }) => fromProcess({
	...options,
	profile
...
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("../_12.mjs").then(({ fromTokenFile }) => fromTokenFile({
