...
import { C as username, b as tanstackStartCookies, f as getCookie, g as openAPI, l as drizzleAdapter, p as getRequestHeaders, r as betterAuth, t as apiKey, u as genericOAuth, v as setCookie, x as twoFactor$1 } from "./vendor-react-DpiQ1Spv.mjs";
...
import { t as getLocale } from "./locale-C4um8W31.mjs";
...
var S3StorageService = class {
	...
	async ensureClient() {
		...
		const { S3Client, ListObjectsV2Command, PutObjectCommand, GetObjectCommand, DeleteObjectCommand } = createRequire(import.meta.url)("@aws-sdk/client-s3");
	...
	async delete(keyOrPrefix) {
		...
		const deleteCommands = keys.map((k) => new this.commands.DeleteObjectCommand({
			Bucket: this.bucket,
			Key: k
		}));
	...
	async healthcheck() {
		try {
			await this.ensureClient();
			const putCommand = new this.commands.PutObjectCommand({
				Bucket: this.bucket,
				Key: "healthcheck",
				Body: "OK"
			});
