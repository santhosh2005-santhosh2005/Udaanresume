...
// Lazy load ImportResumeDialog to avoid SSR issues with pdfjs-dist (uses browser-only APIs)
const ImportResumeDialog = lazy(() => import("./resume/import"));

const ImportResumeFallback = () => (
...
export function DialogManager() {
	...
	const dialogContent = match(activeDialog)
		...
		.with({ type: "resume.import" }, () => (
			<Suspense fallback={<ImportResumeFallback />}>
				<ImportResumeDialog />
			</Suspense>
		))
